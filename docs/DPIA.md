...
| **R-15** | Development and production share one database | Medium | **Mitigated — half of it only since 2026-09-22** | Two controls were intended here and only one worked. The refusal — the suite will not run destructively against a remote `DATABASE_URL` without `TEST_ALLOW_SHARED_DATABASE=true` — worked as written. The redirection did not: `tests/helpers/server.ts` resolved `TEST_DATABASE_URL` and handed it to the server it spawns, but `server/load-env.ts` lets `.env` beat the environment in development, so the child overwrote it and connected to the instance in `.env`. `TEST_DATABASE_URL=<disposable> npm test` therefore pointed the *assertions* at the disposable database and the *server under test* at the one serving the application, which is where its users, scans, consents and appointments were created and deleted. Measured on 2026-09-22 by starting a child exactly as the harness does and printing the resolved host. Fixed by exempting `DATABASE_URL` from the `.env` override for a harness-started process only (`HEALTHAI_TEST_HARNESS`), so `.env` still wins for ordinary development — verified in both directions. Residual: separate instances are still to be provisioned, and rows created by earlier runs of the suite are in the shared database's history. |
...
| **R-19** | A DICOM object sent through the upload path would be stored with its patient-identifying tags intact | **High** (latent) | **Mitigated 2026-09-21** — the analysis path and the no-consent path now persist the de-identified object returned by the inference service (identifiers and private tags removed, instance UIDs replaced, dates to the year), and when the service is unavailable nothing is stored rather than the original. `tests/lung-nodule.test.ts` reads the stored bytes back and asserts the original PatientID and UIDs are absent. Residual: the de-identifier is best-effort PS3.15, unvalidated against a reference implementation; burned-in pixel text is refused only when declared. | `inference/dicom_ingest.py` de-identifies the object it renders for the model, but `server/routes.ts` `persistScanImage` stores the bytes as uploaded — the identified original, not the de-identified copy. Found 2026-09-20; no DICOM has been stored by this path yet (the upload store holds JPEGs only). The upload layer accepts DICOM for any modality — a DICOM submitted as a skin scan is converted for the model and then the *original* bytes are stored; a DICOM submitted as a lung scan is refused before storage now that lung is withdrawn. Close by persisting the de-identified object and never the original, with a test that reads the stored file back and asserts `PatientName` is absent. Until then `docs/DEVICE_INTEGRATION.md`'s statement that the original is never written describes the design, not the behaviour. |
| **R-20** | A model may issue verdicts on inputs it has never been measured on, if the out-of-distribution reference was validated on the wrong domain | **High** | **Occurred and mitigated** | Lung model withdrawn 2026-09-20 after 84 of 87 real CT slices passed its screen (`MODEL_CARDS.md`, `docs/pack/FAILURE_MODES.md` F-08). Consent disclosure re-versioned (`2026-09-20.v3`) so that nobody consents to a model that no longer runs. Standing control: every OOD reference validated against real acquisitions of the domain it must refuse, with pre-set bars, before the model serves. |
| **R-21** | A CT series upload writes the identified originals to a temporary directory on the application host, where they outlive the request if anything goes wrong | **High** | **Mitigated 2026-09-22** | A series arrives as a few hundred identified objects that have to be written somewhere: holding a 150 MB study in memory per concurrent request is not an option, so `server/routes.ts` streams them to disk. One staging directory is created per request, outside `uploads/` so no static handler can serve one, and it is removed in a `finally` that runs on success, on a refused series, on an exception and on a multer size or count limit — that last path previously bypassed the handler entirely and left the partial upload behind. The inference service does the same with its own spool (`tempfile.mkdtemp`, removed in a `finally` that also runs when the client disconnects mid-stream). `tests/dicom-series.test.ts` asserts that no staging directory survives an ingestion or a rejected upload. Residual: a hard process kill between the write and the `finally` leaves identified objects on the host's temporary volume until the OS clears it. That volume is inside the same trust boundary as the running application and is not separately encrypted. |
| **R-22** | Remapped DICOM UIDs could be reversed back to the source PACS, or could collide | **Medium** | **Mitigated 2026-09-22** | UIDs are remapped rather than deleted, because deleting them destroys the study/series/instance tree and with it the ability to assemble a series or find a prior. Hashing them in the clear would preserve the tree and hand anyone holding the stored value a join key back to the source PACS, since the UID space one site emits is small enough to enumerate. The mapping is therefore a salted HMAC-SHA256 truncated to 128 bits and emitted under the PS3.5 Annex B.2 root `2.25.` (`inference/uid_remap.py`), which is collision-resistant at the scale of any study volume and not invertible without the salt. `DICOM_UID_SALT` is generated by `scripts/generate-secrets.ts` and is a key: anyone holding it can confirm a guessed UID, so it is handled like the encryption keys and never logged. Residual: without the variable set the service falls back to a per-process salt and records `uid_mapping_scope: 'ingestion'` on the series, so the degradation is visible in the data rather than assumed away — but a deployment left in that state cannot match a prior study. |
| **R-23** | Erasure does not yet reach the imaging tables | **Medium** | **Open** | `server/erasure.ts` covers `medical_scans` and the patient record; `imaging_studies`, `imaging_series` and `imaging_instances` (added 2026-09-22) and their stored objects are not in its sweep yet. The rows hold no direct identifier — the UIDs are remapped and the objects are de-identified — but they are linked to `patient_id` and so are personal information under §1. Close before a series is ingested for a real patient. |
